<template>
  <div class="space-y-6 max-w-2xl">
    <div>
      ...
      <p class="text-gray-500 text-sm mt-1">{{ $t('settings.subtitle') }}</p>
    </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 space-y-4">
      ...
      <div class="flex items-center gap-4">
        <UAvatar :alt="user?.name" size="xl" class="bg-brand-600" />
        <div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-6 space-y-4">
      <h2 class="text-base font-semibold text-gray-900 dark:text-white">{{ $t('settings.preferences') }}</h2>
      <div class="space-y-3">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $t('settings.darkMode') }}</p>
            <p class="text-xs text-gray-500">{{ $t('settings.darkModeDesc') }}</p>
          </div>
          <input
            type="checkbox"
            :checked="isDarkMode"
        ...
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $t('settings.language') }}</p>
            <p class="text-xs text-gray-500">{{ $t('settings.languageDesc') }}</p>
          </div>
    ...
    <div class="bg-white dark:bg-gray-900 rounded-xl border border-red-200 dark:border-red-900 p-6 space-y-4">
      ...
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ $t('settings.signOut') }}</p>
          <p class="text-xs text-gray-500">{{ $t('settings.signOutDesc') }}</p>
        </div>
