<template>
  ...
  >
    <div class="h-16 flex items-center px-4 border-b border-gray-800">
      <NuxtLink :to="localePath('/dashboard')" class="flex items-center gap-3 min-w-0">
        <UIcon name="i-heroicons-building-office-2" class="w-7 h-7 text-brand-400 flex-shrink-0" />
        <span v-if="!uiStore.sidebarCollapsed" class="font-bold text-lg truncate">{{ config.public.appName }}</span>
      </NuxtLink>
    </div>

    <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto" aria-label="Sidebar navigation">
    ...
    <div class="border-t border-gray-800 p-3">
      <div v-if="!uiStore.sidebarCollapsed" class="flex items-center gap-3 px-3 py-2">
        <UAvatar :alt="user?.name" size="sm" class="bg-brand-600 flex-shrink-0" />
